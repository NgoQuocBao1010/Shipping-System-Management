...
def getShipMoney(request):
    ...
    if not latitude or not longitude:
        return JsonResponse(status=status.HTTP_400_BAD_REQUEST, data={"error": "Invalid query parameters"})
    ...
    if not routingResult:
        return JsonResponse(status=status.HTTP_409_CONFLICT, data={"error": "Could not calculate route"})
